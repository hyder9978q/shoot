...
void main() {
  testWidgets('قسم العب اليوم: يعرض أقرب وقت فاضي ويتحدث بعد الحجز', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: const Directionality(
          ...
          child: Scaffold(body: Column(children: [PlayNowSection()])),
        ),
      ),
    );
    ...
    // — يبين الحين بنظام ١٢ ساعة: ٨:٠٠ مساءً
    expect(find.text('العب اليوم'), findsOneWidget);
    expect(find.text(TimeLabels.hour12(20)), findsWidgets);
    ...
    final padel = FieldsService.instance.search().firstWhere(
      (f) => f.id == 'f3',
    );
    final slots = await BookingsService.instance.slotsFor(
      padel,
      BookingsService.todayDate(),
    );
